...
// light and material
var lightPosition = vec4(-10, -10, 20, 0 );
...
function GenerateSphere(radius, slices, stacks)
{
    ...
    var m=rotate(360/slices, 0, 1, 0);  // m is the rotation matrix, angle of rotation = 360/slices = 360/24=15
    for (var i=1; i<=slices; i++) {
        ...
        triangle(prev[stacks-1], prev[stacks], curr[stacks-1]);
...
function GenerateCone(radius, height)
{
	...
	for (var p=0; p<=stacks; p++)  {
	    ...
	    //line.push(vec4(p*hypotenuse/stacks*sinTheta, p*hypotenuse/stacks*cosTheta, 0, 1));
    }
